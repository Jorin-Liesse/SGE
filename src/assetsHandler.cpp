...
void AssetsHandler::LoadTexture(const string &filePath)
{
    ...
    if (!texture)
    {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Texture Load Error", ("Failed to load texture: " + filePath).c_str(), nullptr);
        return;
    }
...
void AssetsHandler::LoadFont(const string &filePath, int fontSize)
{
    ...
    if (!font)
    {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Font Load Error", ("Failed to load font: " + filePath).c_str(), nullptr);
        return;
    }
...
void AssetsHandler::LoadMusic(const string &filePath)
{
    ...
    if (!music)
    {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Music Load Error", ("Failed to load music: " + (m_basePath + filePath)).c_str(), nullptr);
        return;
    }
...
void AssetsHandler::LoadSoundEffect(const string &filePath)
{
    ...
    if (!chunk)
    {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Sound Effect Load Error", ("Failed to load sound effect: " + filePath).c_str(), nullptr);
        return;
    }
